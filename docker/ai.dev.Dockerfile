...
# AETHER MIGRATE — AI service Dockerfile (DEV)
...
RUN uv venv /build/venv && \
    uv pip install --no-cache --python /build/venv/bin/python \
      ...
      packages/assessment packages/planner packages/iac apps/ai
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
    CMD python -c "import urllib.request; urllib.request.urlopen('http://127.0.0.1:8001/livez')"
CMD ["python", "-m", "uvicorn", "ai.main:app", "--host", "127.0.0.1", "--port", "8001"]
