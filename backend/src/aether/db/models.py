...
class AssistantSettings(Base):
    """Per-workspace assistant configuration. ``egress_mode`` decides what may leave
    the platform: everything, a redacted projection, or nothing (local model only)."""
    ...
    egress_mode: Mapped[str] = mapped_column(String(24))
...
class Conversation(Base):
    ...
    __table_args__ = (Index("ix_conversations_owner", "workspace_id", "user_id", "updated_at"),)
    ...
    egress_mode: Mapped[str] = mapped_column(String(24))
    redaction_vault: Mapped[dict[str, Any]] = mapped_column(default=dict)
...
class ConversationMessage(Base):
    ...
    content: Mapped[dict[str, Any]] = mapped_column()  # model view (possibly redacted)
    display: Mapped[dict[str, Any]] = mapped_column()  # user view
...
class LlmCall(Base):
    """One model request: usage and latency only. Prompt content is never logged here."""
