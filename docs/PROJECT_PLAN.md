...
| | |
|---|---|
| **Status** | In implementation — Phases 0–7 complete (2026-09-26): read-only v1.0 scope delivered for AWS → Azure; see §0.3 |
...
| 7 — Planner + IaC | **done** | Deterministic plans (waves by LB dependency, steps with pre/post-checks and compensation, downtime, rollback), SHA-256 content hash, immutable content (DB trigger), versions/revise/diff, four-eyes hash-bound approval, exports (JSON, Markdown, OpenTofu zip validated with `tofu validate` against azurerm 4.x) |
| 3 — Assistant | **done** | Typed tool registry (14 tools over the REST endpoint functions, plus `workspaces_list` for MCP, RBAC per call, no cloud mutation); separate `assistant` service as the only LLM-egress container; Claude via the official SDK (streaming, adaptive thinking, prompt caching, refusal fallbacks) and local Ollama on an offline network; per-workspace egress policy with reversible redaction; injection guard with audit; SSE chat with result cards; budgets, usage log, 30-day retention; MCP server (streamable HTTP, OAuth via Keycloak) — see [ADR 0007](adr/0007-assistant-architecture.md) |
